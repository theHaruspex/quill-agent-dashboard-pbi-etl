// Best-effort .env load in local dev contexts
try {
...
export function startIngestServer(options?: { port?: number }): Server {
  const port = Number(options?.port ?? process.env.PORT ?? 3000);
  const server = createIngestServer();
  server.listen(port, () => {
    // eslint-disable-next-line no-console
    console.log(`ingest dev server listening on http://localhost:${port}`);
  });
  return server;
}
